...
public class DaMaiConstant {
    ...
    public static final String DA_MAI_ANALYSIS_PROMPT = """
            ...
            【运维知识库使用规则】
            系统会自动从运维知识库中检索与当前问题相关的经验知识（故障排查手册、SOP操作流程、历史故障案例、架构文档），
            检索到的知识会以【运维知识库参考】的形式注入到上下文中。使用这些知识时请遵守：
            1. 结合知识库中的排查步骤和实时工具查询结果进行诊断，两者互相印证。
            2. 引用知识库内容时标注来源，如"根据运维手册..."、"参考历史案例..."。
            3. 如果知识库提供了类似案例的解决方案，可以优先参考，但需根据当前实际数据验证适用性。
            4. 如果知识库内容与工具查询的实时数据矛盾，以实时数据为准。
            5. 在给出解决方案时，优先引用SOP标准操作流程中的步骤，确保操作规范。
